...
A bash scripting automation tool that can deploy web applications to any one of AWS, Azure or Google clouds.

This project primarily aim to create a structure of file system and folders whereby automation scripts written primarily in aws cli, az cli, or gcloud cli by RnD engineers can be grouped together under one "roof". The automation scripts thus created can be used by deployment engineers to deploy different web applications to either AWS, Azure or Google cloud. The end user of this automation script only need to remember simple commands. For example:
...
With this structure, each RnD engineer can choose his or her preferred cli and configuration management tools to automate his infrastructure deployment script. The script that he creates can be used by other RnD engineers as well, either by expanding or modifying existing content, or making a copy and modify in separate sub-folders.

(i) For example, Engineer_A may be more familiar with gcloud CLI and salt, he thus choose these combination to write his deployment script for app01. In some subfolder under this one big folder.

(ii) Engineer_B may be more familiar with Azure CLI and ansible, and thus he choose these combinations to write his deployment scripts for app02. Also in some subfolder under this one big folder.
...
The deployment engineers might receive request to deploy app01 for company XYZ to aws . All he need to do is e.g. deploy XYZ app01 aws. If at a later date company XYZ decided to deploy app01 to Azure, then all the engineer need to do is to issue the following commands:

decommission for_company_A app01 aws

deploy for_company_A app01 azure

Notice that deployment engineers only need a rudimentary knowledge of different clouds available. They do not even need to know which managment configuration tool is being use behind the scene, as these are mostly decided by the RnD engineers who write the automation scripts. However, knowledge of CLI and management configuration tool can still be useful if something failed to work, and cryptic error message appear on the screen, requiring further troubleshooting. This happen all the time.
...
(1) always include at least one monitoring tool in your deployment e.g. zabbix, grafana, or prometeus so that operation engineers can use this monitoring tool to monitor every component in the application environment that they deployed

(2) Daily/weekly/monthly cost/billing information should be made available. May be using the monitoring tool mentioned in (1) above. We want to be able to see cost per cloud, per region, per customer, per resource etc.
